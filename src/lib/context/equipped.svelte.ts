...
export function providedEquipped(initial?: EquippedMap): EquippedAPI {
	...
	const api: EquippedAPI = {
		equipped,
		set: (slot, item) => { equipped[slot] = item; },
		unset: (slot) => { delete equipped[slot]; },
		clear: () => {
...
export function useEquipped(): EquippedAPI {
	...
	if (!ctx) {
		throw new Error('Equipped context not found. Did you call provideEquipped() in +layout.svelte?');
	}
	return ctx;
}
