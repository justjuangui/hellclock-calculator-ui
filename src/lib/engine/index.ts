...
export class Engine {
	...
	onceReady(timeoutMs = 20000): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			const t = setTimeout(() => reject(new Error('Engine timeout')), timeoutMs);
	...
	build(actor: unknown, target: unknown, opts?: { timeoutMs?: number; signal?: AbortSignal }) {
	...
	explain(payload: unknown, opts?: { timeoutMs?: number; signal?: AbortSignal }) {
