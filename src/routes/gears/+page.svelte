<script lang="ts">
  ...
  const { equipped, set: setEquipped, unset: unsetEquipped } = useEquipped();
  ...
  let slotLabel = $derived.by<Record<GearSlot, string>>(() => {
    ...
    for (const s of allSlots) {
      let slotNameKey = gearsHelper.getGearSlotDefinition(s, true)?.slotNameKey;
      if (!slotNameKey) {
          ...
          {#each allSlots as s}
            ...
            >
              ...
              <div
                ...
                data-tip={tooltipText(equipped[s])}
              ></div>
              {#if equipped[s]}
                <span
            ...
            {#each (itemsBySlot[selectedSlot] ?? []).filter( (i) => matches(i, search), ) as item}
              ...
              >
                <div class="card-body p-3 gap-2 flex">
                  ...
                  <!-- stat lines -->
                  <ul class="text-xs mt-1 space-y-1 flex flex-col grow">
                    {#each item.mods as m}
                      <li class="badge badge-soft badge-accent truncate">
                        {fmtValue(m)}
                      </li>
                    {/each}
                    {#if item.mods.length === 0}
                      <li class="opacity-60">No modifiers</li>
                    {/if}
                  </ul>
                  ...
                  <div class="mt-1">
                    <button
                      onclick={() => equip(item)}
                      class="btn btn-primary btn-sm w-full">Equip</button
                    >
                  </div>
                </div>
              </div>
